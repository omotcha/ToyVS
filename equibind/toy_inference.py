...
platform: win
env: toyvs
name: toy_inference.py
bundle equibind inference and ecif scoring without using autogluon tabular predictor
"""
...
from process_mols import read_molecule, get_lig_graph_revised, get_lig_graph_revised_mini, \
...
def inference_and_score_mini(lig_id_list):
    """
    ...
    err_ids = []
    rec_path = os.path.join(protein_path, '7AMA.pdb')
    ...
    for j in lig_id_list:
        ...
        try:
            lig_graph = get_lig_graph_revised_mini(
...
def modulator(n_workers=4):
    ...
    if not only_process_err:
        ...
        pool.starmap(inference_and_score_mini, zip(worker_tasks))
